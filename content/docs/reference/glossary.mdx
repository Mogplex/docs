---
title: Glossary
description: Definitions for Mogplex product terms such as agents, runs, projects, installations, triggers, assignments, automations, sandboxes, worktrees, and observability.
---
...
[Mogplex MCP](/mcp), and the [External MCP server catalog API](/reference/api#external-mcp-servers).
...
[API Models](/reference/api#models) for API behavior.
...
Read next: [API Runs](/reference/api#agent-runs) and [CLI Export](/cli/reference/export).
...
Read next: [API Quickstart](/reference/api#quickstart).

## Worktree

A task-specific Git checkout inside selected Sandbox compute.

Each Worktree binds one mission task, Git branch, checkout path, and Sandbox.
Mogplex creates one only when a planned coding task needs an isolated checkout.
Starting a Sandbox, opening a preview, or running a command does not create a
Worktree.

Archive keeps the checkout, branch, record, and Sandbox compute. Prune removes
the archived checkout and task binding. It keeps the Git branch, pruned record,
and Sandbox compute.

A Mogplex Worktree is not a Sandbox snapshot, browser snapshot, run, workspace
session, branch, or local contributor worktree.

Read next: [Worktrees](/web/worktrees) and
