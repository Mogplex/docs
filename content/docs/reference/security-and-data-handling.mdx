...
risk, hidden command failures, and whether final claims match the tool log.
Those judgments need short excerpts of commands, tool output, and the agent's
final message. Secrets are removed before anything is sent or stored.
