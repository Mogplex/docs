...
Use [API Authentication](/reference/api#authentication) for request examples and
scope behavior.
...
Mogplex judges agent work as it happens with a small evaluation model: command
risk, hidden command failures, whether final claims match the tool log, and
which of the memories loaded for a request, and which of the skills available
to it, the request needed. Those judgments need short excerpts of commands,
tool output, and the agent's final message, plus your request with the
memories loaded for it and summaries of the skills available to it. Secrets
are removed before anything is sent or stored.

This is a choice. A team owner or admin can turn **Run checks** off for the
team, and any person can turn it off for their work outside a team. With it
off, nothing is sent to the evaluation model and no judgment is recorded. See
[Settings](/web/settings#run-checks) for what stops working when it is off.
...
and the [External MCP server catalog API](/reference/api#external-mcp-servers) for the exact behavior.
...
<Cards>
  <Card title="API Authentication" href="/reference/api#authentication" />
