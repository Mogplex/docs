...
from any shell. They wrap the [v1 runs API](/reference/api#agent-runs) one-for-one and
...
Issue one at [mogplex.com/settings/api-keys](https://mogplex.com/settings/api-keys).
...
[`StartMogplexApiAgentRunResult`](/reference/api#start-a-run) object with
...
`--json` emits the [`MogplexApiRunDetail`](/reference/api#get-a-run) object
...
One [`PresentedAiCallEvent`](/reference/api#list-run-events) per line. The
...
- [API → Runs](/reference/api#agent-runs) — the REST endpoints this command wraps
- [API → Errors](/reference/api#errors) — error codes mapped to exit codes
