...
| Coding task uses the wrong checkout | selected Sandbox, task identity, Worktree binding, or checkout path |
...
9. Check [Worktrees](/web/worktrees) when a planned coding task uses an
   isolated checkout.
...
Use [API Models](/reference/api#models) only when you need the route behavior, not the
...
## Worktree or checkout is wrong

Start with
[Control Execution Environments](/guides/control-execution-environments).
Confirm that the task used the selected Sandbox and an active persisted
Worktree.

Check these values separately:

- mission and task ID
- Worktree record ID and status
- Git branch
- checkout path
- Sandbox record ID
- provider runtime ID

Do not infer a Worktree from a Sandbox branch. Do not move a worker to another
checkout to work around a binding error.

If the Sandbox is stopped, restart the same Sandbox record before using the
checkout. If the Sandbox record is gone, follow
[Worktree failure recovery](/web/worktrees#failure-recovery).

## API request fails

Start with [API Authentication](/reference/api#authentication) and
...
Use [API Quickstart](/reference/api#quickstart) for the shortest complete path from
