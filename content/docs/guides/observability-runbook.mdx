...
description: Trace failed, stuck, expensive, or misrouted work through runs, tools, selected Sandboxes, Worktree checkouts, and API state.
---
...
**Did Mogplex create a run, call, Sandbox, or Worktree record?**
...
- Worktree ID, task ID, and checkout path if isolated coding started
- model and provider
- first error message
...
8. task, Worktree, branch, and checkout context when present
9. resource decision source or mismatch reason
10. raw metadata only if the higher-level fields are not enough
...
1. Open **Connections** in the app sidebar and test the connection.
...
[External MCP server catalog API](/reference/api#external-mcp-servers) and local CLI auth separately.
...
## Worktree Or Checkout Failure

For isolated coding failures:

1. Confirm that the mission created the intended task ID.
2. Confirm that the task has one active persisted Worktree.
3. Confirm that its Sandbox binding matches the selected Sandbox.
4. Confirm that the worker used the exact persisted checkout path.
5. Check the branch and Worktree status before Diff or Rebase.
6. Use Archive or Prune only for Worktree lifecycle changes.

Use [Control Execution Environments](/guides/control-execution-environments)
for the decision contract. Use [Worktrees](/web/worktrees#failure-recovery) for
stopped, missing, stale, archived, or pruned resources.
