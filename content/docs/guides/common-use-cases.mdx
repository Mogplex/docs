...
Use the [API Reference](/reference/api) when another system needs to list repos,
start runs, inspect run events, read model metadata, or fetch runtime state.

Use [API Authentication](/reference/api#authentication) before wiring clients. Use
[API Errors](/reference/api#errors) when you need response shape and status behavior.
Use [API Quickstart](/reference/api#quickstart) when you want the shortest complete
token-to-run path.
