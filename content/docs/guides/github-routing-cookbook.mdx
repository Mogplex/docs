...
| Scheduled maintenance | Automation with a schedule trigger and Task node, or a cron assignment |
...
Use an [automation through MCP](/mcp/automations#scheduled-tasks) with a
schedule trigger and Task node for repository checks and new pull requests.
[Assignments](/web/assignments) also support recurring work for one repo.
...
or [API Quickstart](/reference/api#quickstart) when GitHub Actions or another system
