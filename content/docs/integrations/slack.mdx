...
1. Open **Connections** in the app sidebar and find the **Slack** section.
...
Write `$handle` anywhere in a message to `@mogplex` to have the agent follow
one of your skills, for example `@mogplex review this with $release-notes`.
Slack reads a message that starts with `/` as one of its own commands, so the
`/handle` spelling does not work here. The skills are those of the Mogplex
user your Slack identity maps to. See [Using Skills](/web/agents/using-skills).
