...
Mogplex exposes 28 user-scoped tools. The client loads their current JSON
...
| `mogplex_list_skills` | `read` | Search the user's skills. Returns handle, name, and description. |
| `mogplex_get_skill` | `read` | Read one skill's full instructions by handle. |
...
A run prompt can invoke a skill without fetching it: write `$handle` in the
`prompt` of `mogplex_start_agent_run`. See
[Using Skills](/web/agents/using-skills).
...
| `mogplex_get_automation_schema` | `read` | Read graph fields, validation rules, and scheduled-task examples. |
| `mogplex_validate_automation` | `read` | Check a complete graph without saving or running it. |
...
| `mogplex_delete_automation` | `write` | Permanently delete an automation that has no in-flight runs. |
...
| `mogplex_cancel_automation_run` | `write` | Cancel a Flow job using its automation ID and job run ID. |
...
| `mogplex_cancel_run` | `write` | Request cancellation of a one-off agent run, not a Flow job. |
