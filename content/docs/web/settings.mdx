...
- whether the agent's final claims, such as "tests pass" or "PR opened", match
  what its tools did
- which of the memories and agent skills loaded for a request the request
  actually needed. This is recorded only and does not change what is loaded.
...
request, together with the memories and skill summaries loaded for it.
