...
<Callout type="info">
  Local automation clients and the Mogplex MCP server use
  `GET /api/v1/mogplex/models`. That PAT-authenticated route returns only the
  enabled, visible models the current user can actually reach. The
  `mogplex_list_models` MCP tool wraps it directly.
</Callout>
