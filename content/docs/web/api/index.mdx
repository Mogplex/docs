...
- [API Quickstart](/web/api/quickstart) for the shortest complete path from
  ...
  listing
- [Authentication](/web/api/authentication) for MCP OAuth, PAT lifecycle,
  scopes (`read` / `write`), idempotency, and rate limits
...
2. OAuth bearer token on `/api/v1/mogplex/mcp`
3. internal Playwright auth headers used in end-to-end testing
4. browser session auth from the signed-in web app
...
- Many shared read or control-plane routes accept PAT auth through the common
  auth helper.
- The outbound MCP endpoint accepts OAuth access tokens as well as PATs.
