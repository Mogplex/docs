...
<Cards>
  ...
  <Card title="Connections" href="/configure-and-extend/connections-and-mcp" />
...
- **Worktrees** in Control shows task-specific Git checkouts inside selected
  Sandbox compute. Its count is independent from the Sandbox count.
...
- **Settings** opens a secondary sidebar with separate pages for account,
  teams, keys, and billing. Billing has **Billing Settings** and
  **Usage** tabs.
...
- use [Control Execution Environments](/guides/control-execution-environments)
  when a task must choose compute, an isolated checkout, or a lifecycle action
...
- **Where do I set up models and external tools?** Use
  [Settings](/web/settings) for account access,
  [Connections](/configure-and-extend/connections-and-mcp) for external services, and
...
- **Why do Sandbox and Worktree counts differ?** They count compute and task
  checkouts. Starting compute does not create a Worktree. Read
