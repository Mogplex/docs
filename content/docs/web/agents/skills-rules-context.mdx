...
Skills in your library are usable on every surface. You can invoke one by name
with `/handle` or `$handle`, and agents can search and load them on their own.
See [Using Skills](/web/agents/using-skills).
