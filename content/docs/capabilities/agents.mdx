...
## How Control agents use execution resources

Control agents select resources from the requested outcome. Preview and shell
work use Sandbox compute without a Worktree. A planned coding task gets a
Worktree only when it needs an isolated checkout.

A worker starts after the task has an active persisted Worktree. It uses that
Worktree's exact Sandbox binding and checkout path. Sandbox lifecycle actions
do not archive or prune Worktrees.

Read [Control Execution Environments](/guides/control-execution-environments)
for the complete agent decision contract.
