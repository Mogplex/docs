...
The server must support Streamable HTTP and have a public URL. Saved secret headers stay on the server.
...
Team capability checks still apply. Saved `enabled_tools`, `disabled_tools`, and per-tool restrictions in **Extra JSON** also apply to chat.
Tools with an explicit `prompt` approval mode require Control, which can ask before a call. Other chat surfaces withhold them.
...
Enabled public HTTP servers from **MCP Servers** also load in Control, workspace chat, the Slack agent, and native-harness runs.
...
This existing cap does not apply to the **MCP Servers** catalog.
