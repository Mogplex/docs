...
description: Configure REST and MCP connections, understand scope resolution, sync cloud MCP servers to the CLI, and distinguish inbound tools from the outbound Mogplex MCP server.
---
...
| [Mogplex MCP server](/extend/mcp-server) | External MCP clients call Mogplex | Let local agents operate repos, sandboxes, automations, models, runs, and logs. |
...
[Mogplex MCP server](/extend/mcp-server). Use [Skills](/extend/skills) when the
