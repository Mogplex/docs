...
### Presets that run next to your code

Trigger.dev publishes its MCP server as a local command, not a hosted URL, and
tools such as `deploy` and `start_dev_server` need your project files. Mogplex
starts it where the code is:
...
Chat turns that run on Mogplex servers have no filesystem to launch it in, so
they do not load this preset. Hand Trigger.dev work to a sandbox agent.

To add it, create a Personal Access Token at
[cloud.trigger.dev/account/tokens](https://cloud.trigger.dev/account/tokens)
and paste it into the preset. It starts with `tr_pat_`. **Test** confirms that
Trigger.dev accepts the token; it does not start the server. Mogplex fixes the
command in the preset, so nothing you type is ever executed.
