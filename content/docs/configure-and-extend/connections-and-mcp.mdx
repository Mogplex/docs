...
**API tools that need no sandbox.** Control, workspace chat, the Slack agent, and
runs on the native Mogplex harness get the Trigger.dev management API as tools,
read and write, called directly from Mogplex servers:
...
| --- | --- |
| Control | Every enabled connection in scope for the selected repo. |
...
| --- | --- |
| Control | Each call pauses on an approval card. Approve to run it, deny to skip it. Every request is recorded with your decision. |
