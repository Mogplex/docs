...
The test has a six-second deadline. A successful connection with no tools shows a count of zero.
...
A tool disabled in saved settings stays disabled until its switch is on. Other block rules still apply.

Save changes to apply them on the next turn. The controls preserve unrelated CLI fields and permissions.
The app rejects invalid permissions. Correct invalid permission fields in **Extra JSON**. Invalid policies never become permissive defaults.
...
| Saved MCP tool missing in web chat | Choose Test connection. Check the result and each tool's permission. Enable the server to use its allowed tools. |
