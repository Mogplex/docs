...
Servers remain personal. In team scope, owners, admins, and developers can use them. Viewers cannot use connection tools.
...
Per-tool `deny` excludes a tool. `prompt` requires a Control approval card and withholds the tool on other web chat surfaces.
